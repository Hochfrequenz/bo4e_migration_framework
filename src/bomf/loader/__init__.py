...
@attrs.define(auto_attribs=True, kw_only=True)
class LoadingSummary(ABC, Generic[_TargetEntity]):
    """
...
class EntityLoader(ABC, Generic[_TargetEntity]):
    """
    ...
    async def load(self, entity: _TargetEntity) -> LoadingSummary:
        ...
        loaded_at: datetime
        try:
            entity_loading_result = await self.load_entity(entity)
            if entity_loading_result is None:
